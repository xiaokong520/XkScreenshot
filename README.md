...
把选区钉在屏幕上，可以拖动、滚轮缩放、Ctrl+滚轮调透明度、Del 关闭，右键菜单可复制、另存、
按 90° 整角旋转。

鼠标直接抓图片的四个角往外拖就是旋转：光标挪到角上会变成旋转光标，拖拽绕图片中心转，松手即定。
拖到 90° 整数倍附近（3° 以内）会自动吸附进去，按住 Shift 改成 15° 一格 —— 摆正歪掉的截图主要
靠这个。细调还有 Alt+滚轮一格 1°（按住 Shift 一格 15°），Ctrl+R 把角度归零。

角度是任意值。转过的贴图四角是透空的，鼠标点上去会穿到下层窗口。

按 F3 钉的是剪贴板里的内容，支持图片、单个图片文件、以及文本（会排版成一张图）。剪贴板里
没有可贴的内容时，钉上一次截的那张图。

贴图可以存档。设置里打开「重启后恢复贴图」之后，退出时屏幕上的那几张贴图会被记下来，下次
打开程序按原位置、原大小、原旋转角度和透明度摆回去；配合「开机自动启动」，重启电脑也一样。
默认是关的 —— 随手贴、随手关的人不会指望程序替他记着这些。关掉这一项会把存档一并清掉。
...
<程序目录>\pins\                          贴图存档（index.json + 贴图画面，见「重启后恢复贴图」）
